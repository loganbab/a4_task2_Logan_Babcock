...
ggplot(data = yearly_count, aes(x = Year, y = Count)) + # makign a column chart to display counts
  ...
  labs( caption = expression(paste(bold("Figure 1:"), " Number of juvenile hares caught during sampling from 1998-2012, with bar height representing number of individual hares."))) # adding figure caption
...
hare_weights <- hares_raw %>% # make a new data set with variables site sex and weight
  ...
  mutate(full_sex = case_when( # expanding full sex 
    sex == "f" ~ "Female",
    sex == "m" ~ "Male"))
...
ggplot(data = hare_weights, aes(x = full_sex, y = weight)) + #exploratory scatter plot graph
  geom_jitter(aes(color = full_sex)) +
  facet_wrap(~ site) + # adding site as a face wrap for comparison
  labs(x = "Sex", y = "Weight (g)", caption = expression(paste(bold("Figure 2:"), "This plot shows the distribution of hare weights for each sex, faceted by survey site.")))

# histogram

ggplot(data = hare_weights, aes(x = weight, fill = full_sex)) + # exploratory histogram of weights with sex stacked
  ...
  labs(x = "Weight (g)", y = "Percent of sample", caption = expression(paste(bold("Figure 3:"), "This histogram shows the distribution of hare weights by percent of population, and is faceted by site"))) + # adding labels
  theme_light()
...
ggplot(data = weight_foot, aes(x = hindft, y = weight)) + # making a graph with the variables
  ...
  labs(x = "Hindfoot Length (mm)", y = "Weight (g)", caption = expression(paste(bold("Figure 4:"), "Hindfoot lenth (mm) and hare weight in grams are plotted to visualize their relationship")))
